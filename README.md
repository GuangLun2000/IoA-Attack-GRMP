...
- **`--stable`**: greedy decoding; default `max_new_tokens` is **64** for `default` / `simple`, **72** for **`strict`**, **96** for **`strict_json`** (override with `--max-new-tokens`). Decoding runs until **EOS** or **`max_new_tokens`** (no early stop on “two lines” for single-pass strict/json). Default `repetition_penalty` is **1.1**, or **1.15** with **`strict`** / **`strict_json`** (unless you set `--repetition-penalty`).
